...
\begin{itemize}[topsep=1pt, itemsep=0pt, parsep=2pt]
  ...
  \item \textbf{Coordination Retention:} By tagging the conjunction \texttt{or} as I-NP, the pipeline avoids splitting the entity into two fragmented objects. This prevents \texttt{participation} from floating without its full ownership context, which is a vital prerequisite for accurately training the Named Entity Recognition model in Task~2.1.
\end{itemize}
...
The final output aggregates this information into a nested JSON array, where each entry contains a segmented clause and a list of its token-level dependency mappings:
\begin{minted}[frame=lines, fontsize=\scriptsize, breaklines]{json}
{
...
Using the Sponsorship Agreement as a structural baseline, the parser correctly identifies the relational triad of actors, actions, and objects. In the parsed clause ``ISO may terminate this Agreement,'' the algorithm successfully designates \texttt{terminate} as the grammatical \texttt{ROOT}.

\medskip

The dependency tree accurately maps the surrounding entities to this core predicate:

\begin{itemize}[topsep=0pt, parsep=0pt, itemsep=0pt]
  \item \textbf{Nominal Subject (nsubj):} ``ISO'' is correctly identified as the acting party and assigned ``terminate'' as its syntactic head.
  \item \textbf{Direct Object (dobj):} ``Agreement'' is correctly identified as the target of the action, also pointing directly to the root verb.
  \item \textbf{Determiner (det):} The modifier ``this'' is correctly attached to its local head (``Agreement'') rather than floating independently.
\end{itemize}

This proves that the transition-based parser reliably captures the core intent of a legal clause. By anchoring the acting party and the target object to the predicate, the system preserves the legal logic even when clauses are inverted or padded with complex modifiers.
...
The resulting JSON arrays provide a pristine, machine-readable foundation. This output strictly fulfills the final structural requirement of Task 1, cleanly concluding the Processing and Syntax Analysis phase and preparing the text pipeline for the Machine Learning and Classification objectives in Assignment 2.
